...
* [Using SDK through `BlinkCardScanActivity`](#using-scan-activity)
...
3. In `File` dialog select _BlinkCard_ folder.
...
### Adding _BlinkCard_ SDK dependency

The `BlinkCard` library is available on Maven Central repository.
...
Add _BlinkCard_ as a dependency in module level `build.gradle(.kts)`:
...
```
data class BlinkCardScanningResult(
    val issuingNetwork: String,
    val cardAccounts: List<CardAccountResult>,
    val iban: String?,
    val cardholderName: String?,
    val overallCardLivenessResult: CheckResult,
    val firstSideResult: BlinkCardSingleSideScanningResult?,
    val secondSideResult: BlinkCardSingleSideScanningResult?
)
```
...
_BlinkCard_ SDK is distributed with **ARMv7** and **ARM64** native library binaries.

_BlinkCard_ is a native library written in C++ and available for multiple platforms. Because of this, _BlinkCard_ cannot work on devices with obscure hardware architectures. We have compiled SDK's native code only for the most popular Android [ABIs](https://en.wikipedia.org/wiki/Application_binary_interface).

If you are combining _BlinkCard_ library with other libraries that contain native code in your application, make sure to match the architectures of all native libraries. For example, if the third-party library has only ARMv7 version, you must use exactly ARMv7 version of _BlinkCard_ with that library, but not ARM64. Using different architectures will crash your app at the initialization step because JVM will try to load all its native dependencies in the same preferred architecture and fail with `UnsatisfiedLinkError`.

To avoid this issue and ensure that only architectures supported by the _BlinkCard_ library are packaged in the final application, add the following statement to your `android/defaultConfig` block inside `build.gradle.kts`:
...
There are two primary methods for integrating the _BlinkCard_ SDK into your Android application: via the `BlinkCardCameraScanningScreen` composable or the `BlinkCardScanActivity` activity. Each approach offers distinct advantages and trade-offs. The following guidelines can help determine the most suitable integration method for your use case.

When to use the `BlinkCardCameraScanningScreen` composable:
...
You can use basic customization options in our default `BlinkCardCameraScanningScreen` composable:
...
BlinkCardCameraScanningScreen(
    ...
    onScanningSuccess = { scanningResult ->
        // result is BlinkCardScanningResult
    },
    onScanningCanceled = {
        // user canceled the scanning
    }
...
class YourBlinkCardScanningUxViewModel(
    ...
    val imageAnalyzer = BlinkCardAnalyzer(
            ...
            override fun onUxEvents(events: List<BlinkCardScanningUxEvent>) {
                ...
                for (event in events) {
                    when (event) {
                        ...
                        is BlinkCardScanningUxEvent.CardLocated -> {
                            // TODO
                        }
...
fun YourCameraScanningScreen(
    viewModel: YourBlinkCardScanningUxViewModel
...
For larger control over the UX, you can use the open-source `blinkcard-ux` library and perform certain modifications. **Only the source files that specifically allow for modification by the license header** can be modified.

To do so, you can include the source code of our library directly in your application.
It is located in `libs/sources/blinkcard-ux` module.
...
val blinkCardLauncher = rememberLauncherForActivityResult(
    ...
    onResult = { scanningResult ->
      ...
      // use scanningResult.result (BlinkCardScanningResult)
      }
  }
)
```
...
### Adding _BlinkCard_ Core SDK dependency for low-level API

For low-level API integration, only _BlinkCard_ SDK core library: **blinkcard-core** is needed.
In this case, `blinkcard-ux` is not needed.
...
Add _blinkcard-core_ library as a dependency in module level `build.gradle(.kts)`:
...
1. First initialize the SDK to obtain `BlinkCardSdk` instance by calling `BlinkCardSdk.initializeSdk` suspend function from a Coroutine:
```kotlin
val maybeInstance = BlinkCardSdk.initializeSdk(
    context,
    BlinkCardSdkSettings(
...
```
2. Create `BlinkCardScanningSession` by calling suspend function `BlinkCardSdk.createScanningSession(BlinkCardSessionSettings)`
```kotlin
val scanningSession = blinkCardSdk.createScanningSession(BlinkCardSessionSettings(
...
For more information on how to use `BlinkCardScanActivity`, please refer to the [`Using SDK through BlinkCardScanActivity`](#using-scan-activity) section.
